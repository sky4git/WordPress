<div v-if="gcsauthcheck == 'ready'" class="w2cloud__cloud--sync">
    <div>
    ...
    <div class="w2cloud__button--border">
        <p v-if="processRunning == 'end'">Synchronization complete. Uploaded {{totallUpload}} files. Failed to send {{errorPercentage}} files.</p><p v-else-if="processRunning == 'running'"><span>Warning:</span> <?php _e('Do not reload or close this tab. It may take few minutes.','w2cloud'); ?></p><p v-else><?php _e('Press the sync button to sync all media.','w2cloud'); ?></p><b-button type="submit" @click.prevent="GCS_Sync" variant="primary" class="area-design"> <?php _e('Sync','w2cloud'); ?> </b-button>
    </div>
...
<div v-else class="w2cloud__cloud--sync">
  <div class="w2cloud__cloud-sync-content">
    ...
    <p class="w2cloud__error"><?php _e('Save auth info to sync media on Google Cloud Storage','w2cloud'); ?></p>
